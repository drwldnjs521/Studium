...
if grep -E "in use at exit: [1-9][0-9,]* bytes" $logfile ; then
    echo -e "\tFEHLER: Speicherleck! Details in Datei $logfile."
    exit 4
else
    echo -e "\tOK: Speichertest"
fi
$program "$invalidreadfile" "$tmpfile" 2>/dev/null
result=$?
if [ $result -gt 1 ]; then
    echo -e "\tOK: Rückgabewert ist $result.\n"
else
    echo -e "\tFEHLER: Rückgabewert ist nicht größer 1, sondern $result.\n"
    exit 3
fi
